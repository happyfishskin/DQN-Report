...
# ==== 自動建立資料夾 ====
os.makedirs("../vid_task3_inferance", exist_ok=True)
...
# ==== 建立 Atari 環境 ====
def make_env(env_name="ALE/Pong-v5", render_mode="rgb_array"):
...
def save_evaluation_gif(policy_net, env_name="ALE/Pong-v5", filename="pong_eval_final.gif"):
    ...
    env.close()
    imageio.mimsave(f"../vid_task3_inferance/{filename}", images, fps=30)
    print(f"🎥 模擬影片已儲存為 ../vid_task3_inferance/{filename} | 得分: {total_reward}")
...
def inference():
    ...
    save_evaluation_gif(policy_net, filename="../vid_task3_inferance/pong_eval_infer.gif")
